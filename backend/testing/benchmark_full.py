...
# Dynamic model directory path to backend/medikiosk-translation/models
...
# 2. Load PyTorch Weights from Local Model Folder (FP16 on GPU)
...
print(f"[OK] Model weights loaded into {device.upper()} ({model_dtype}) in {t_load:.2f} ms!")
...
for idx, (lang_name, lang_code) in enumerate(ALL_22_LANGUAGES, 1):
    try:
        ...
        print(f"[{idx:02d}/22] Language: {lang_name:<10} ({lang_code}) | Latency: {elapsed_ms:6.2f} ms | Output Sample: {results[0]}")
    except Exception as e:
        print(f"[{idx:02d}/22] Error in {lang_name}: {e}")
...
if latencies:
    ...
    print("==========================================================================")
    print(f" Total Languages Benchmark Target : 22")
    print(f" Successful Language Evaluations   : {success_count}/22 ({(success_count/22)*100:.1f}%)")
    print(f" Minimum Batch Latency (3 items)  : {min_lat:.2f} ms")
    print(f" Maximum Batch Latency (3 items)  : {max_lat:.2f} ms")
    print(f" Average Batch Latency Across 22  : {avg_lat:.2f} ms")
    print("==========================================================================")
